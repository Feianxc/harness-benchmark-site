import { truncateDigest, type PublicationRecord } from "@ohbp/verifier-core";
import { localizeAdmissionReasons } from "./admission-i18n.js";
import { DEFAULT_UI_LANGUAGE, t, type UiLanguage } from "./i18n.js";
import type { EntryDetailView, EntryResearchView, EntryScorecardView } from "./types.js";

function chooseBaseline(records: PublicationRecord[], current: PublicationRecord): PublicationRecord | undefined {
  const slicePeers = records.filter(
    (record) =>
      record.submission.benchmark.id === current.submission.benchmark.id &&
  ...
  return (
    slicePeers.find((record) => record.submission.harness.id.includes("baseline")) ??
    slicePeers
      .slice()
...
function localizedStateEventSummary(
  event: NonNullable<PublicationRecord["verification_record"]["state_history"]>[number],
  lang: UiLanguage,
): string {
...
function publicationVerdict(
  ...
  if (publicationState === "disputed") {
    return t(
      lang,
      "该结果目前处于 disputed 状态，已从活跃榜单暂时下架，等待平台裁决。",
      "This result is currently disputed and has been temporarily removed from active boards pending platform review.",
    );
  }
  ...
  if (publicationState === "corrected") {
    return t(
      lang,
      "该结果已经被标记为 corrected；修正版本会在重新复核后决定是否回到活跃榜单。",
      "This result is marked corrected; the revised version will be reconsidered for active boards after re-review.",
    );
  }
  ...
  if (publicationState === "invalidated") {
    return t(
      lang,
      "该结果已经被判定为 invalidated，只保留历史说明，不再作为有效比较依据。",
      "This result has been invalidated and is kept only for historical explanation, not for active comparison.",
    );
  }
  ...
  if (publicationState === "archived") {
    return t(
      lang,
      "该结果已归档，仅保留历史页，不再参与当前榜单。",
      "This result has been archived and remains available only as a historical page, not as part of the current boards.",
    );
  }
  ...
  if (publicationState === "rejected") {
    return t(
      lang,
      "该结果未进入公开发布状态，因此不会出现在活跃榜单中。",
      "This result never entered public release, so it does not appear on active boards.",
    );
  }
...
function localizedVerificationRecord(
  verificationRecord: PublicationRecord["verification_record"],
  lang: UiLanguage,
): PublicationRecord["verification_record"] {
  return {
    ...verificationRecord,
    board_admission: {
      official_verified: {
        ...verificationRecord.board_admission.official_verified,
        satisfied_reasons: localizeAdmissionReasons(
          verificationRecord.board_admission.official_verified.satisfied_reasons,
          lang,
        ),
        blocked_reasons: localizeAdmissionReasons(
          verificationRecord.board_admission.official_verified.blocked_reasons,
          lang,
        ),
        next_actions: localizeAdmissionReasons(
          verificationRecord.board_admission.official_verified.next_actions,
          lang,
        ),
      },
      reproducibility_frontier: {
        ...
        satisfied_reasons: localizeAdmissionReasons(
          verificationRecord.board_admission.reproducibility_frontier.satisfied_reasons,
          lang,
        ),
        blocked_reasons: localizeAdmissionReasons(
          verificationRecord.board_admission.reproducibility_frontier.blocked_reasons,
          lang,
        ),
        next_actions: localizeAdmissionReasons(
          verificationRecord.board_admission.reproducibility_frontier.next_actions,
          lang,
        ),
      },
      community_lab: {
        ...
        satisfied_reasons: localizeAdmissionReasons(
          verificationRecord.board_admission.community_lab.satisfied_reasons,
          lang,
        ),
        blocked_reasons: localizeAdmissionReasons(
          verificationRecord.board_admission.community_lab.blocked_reasons,
          lang,
        ),
        next_actions: localizeAdmissionReasons(
          verificationRecord.board_admission.community_lab.next_actions,
          lang,
        ),
      },
    },
...
function publicationPanel(
  record: PublicationRecord,
  lang: UiLanguage,
): EntryResearchView["publication_panel"] {
  return [
    {
...
function buildResearch(record: PublicationRecord, lang: UiLanguage): EntryResearchView {
  ...
  const displayVerificationRecord = localizedVerificationRecord(
  ...
  return {
    ...
    state_history:
      displayVerificationRecord.state_history?.map((event) => ({
        at: event.at,
    ...
    correction_log:
      intakeValidation?.corrections.map((correction) => ({
        field: correction.field,
        reason: correction.reason,
        declared:
          correction.declared === undefined ? undefined : String(correction.declared),
        corrected:
          correction.corrected === undefined ? undefined : String(correction.corrected),
      })) ?? [],
    redaction_notes: [
      sealedMode
      ...
      t(
        lang,
        `release_policy=${record.verification_record.release_policy}，visibility_class=${record.verification_record.visibility_class}。`,
        `release_policy=${record.verification_record.release_policy}, visibility_class=${record.verification_record.visibility_class}.`,
      ),
    ],
...
export function buildEntryDetailView(
  ...
  return {
    ...
    subtitle: t(
      lang,
      `${record.submission.benchmark.id}@${record.submission.benchmark.version} • 赛道 ${record.submission.benchmark.lane_id}`,
      `${record.submission.benchmark.id}@${record.submission.benchmark.version} • lane ${record.submission.benchmark.lane_id}`,
    ),
    summary: {
