...
import type {
  ...
  CompareFrameworkView,
  ConsumerHomePageView,
  ConsumerProvenanceNote,
  ConsumerScoreLevel,
...
interface HarnessProfile {
  ...
  scenarioTagsZh: string[];
  scenarioTagsEn: string[];
  fitByHost: Record<HostId, number>;
  compareValues: {
...
const DECISION_SCORES: Record<HarnessId, DecisionScoreSet> = {
  gsd: {
    newProject: 79,
    existingRepo: 62,
    longTask: 57,
...
const HOST_META: Record<HostId, HostMeta> = {
  general: {
    badgeZh: "通用",
    badgeEn: "General",
    titleZh: "通用总榜",
...
const TOP_CARD_CONFIGS: Record<HostId, TopCardConfig[]> = {
  general: [
    {
      titleZh: "综合首位",
...
const COMPARE_DIMENSION_IDS_HOME: CompareDimensionId[] = [
  "new_project",
  "existing_repo",
  "long_task",
  "setup_speed",
...
const COMPARE_DIMENSION_IDS_FULL: CompareDimensionId[] = [
  "new_project",
  "existing_repo",
  "long_task",
  "setup_speed",
...
function primaryTopCardConfig(hostId: HostId): TopCardConfig {
...
function backupTopCardConfig(hostId: HostId): TopCardConfig {
...
const RANK_EXPLANATIONS: Partial<Record<HostId, Partial<Record<HarnessId, { zh: string; en: string }>>>> = {
  general: {
    openspec: {
...
function consumerProvenance(lang: UiLanguage): ConsumerProvenanceNote {
  return {
    mode: "curated_host_fit_demo",
    title: t(lang, "选型层，非 verifier-backed 真榜", "Selection layer, not a verifier-backed board"),
    body: t(
      lang,
      "当前层只缩小 shortlist；可验证结果、准入状态和 Wilson 区间在 Evidence Boards。",
      "This layer narrows the shortlist only; verified results, admission state, and Wilson intervals live in Evidence Boards.",
    ),
  };
}

function capabilityAverage(profile: HarnessProfile): number {
  return (
...
function overallScore(profile: HarnessProfile, hostId: HostId): number {
  return Math.round(capabilityAverage(profile) * 0.55 + profile.fitByHost[hostId] * 0.45);
}

function decisionScores(profile: HarnessProfile): DecisionScoreSet {
  return DECISION_SCORES[profile.harnessId];
}

function scoreLevel(score: number): ConsumerScoreLevel {
  if (score >= 85) {
    return "very_high";
  }
  if (score >= 75) {
...
function scoreLevelLabel(score: number, lang: UiLanguage): string {
...
function compareDimensionSummary(
  lang: UiLanguage,
  id: CompareDimensionId,
  profile: HarnessProfile,
  score: number,
  ...
  switch (id) {
    case "new_project":
      return score >= 85
        ? t(
            lang,
            `${profile.label} 的规范与任务拆解完整度较高，greenfield 启动适配高。`,
            `${profile.label} has higher structure for greenfield setup.`,
          )
        : score >= 70
          ? t(
              lang,
              `${profile.label} 可覆盖新项目起步，范围收敛后更稳定。`,
              `${profile.label} can cover greenfield starts and stabilizes as scope narrows.`,
            )
          : t(
              lang,
              `${profile.label} 对已有边界的工作适配更高。`,
              `${profile.label} has higher fit when some structure already exists.`,
            );
    case "existing_repo":
      return score >= 85
        ? t(
            lang,
            `${profile.label} 围绕具体改动推进的稳定性较高。`,
            `${profile.label} has higher stability around concrete deltas.`,
          )
        : score >= 70
          ? t(
              lang,
              `${profile.label} 能接住存量仓库，修改边界清晰时更稳定。`,
              `${profile.label} can work in an existing repo, especially once the change boundary is clear.`,
            )
          : t(
              lang,
              `${profile.label} 放在复杂存量仓库里呈现条件型适配。`,
              `${profile.label} is more selective once the repo already has a lot of history and complexity.`,
            );
    case "long_task":
      return score >= 85
        ? t(
            lang,
            `${profile.label} 对长链路任务里的节奏、角色和上下文保持度较高。`,
            `${profile.label} has higher retention of pace, roles, and context across long task chains.`,
          )
        : score >= 70
          ? t(
              lang,
              `${profile.label} 可覆盖较长任务，重型持续编排维度较低。`,
              `${profile.label} can cover longer tasks, with lower fit for heavy continuous orchestration.`,
            )
          : t(
              lang,
              `${profile.label} 短回合推进适配更高。`,
              `${profile.label} has higher fit for shorter bursts than marathon-style delivery.`,
            );
    case "setup_speed":
      return score >= 85
        ? t(
            lang,
            `${profile.label} 前置配置少，产出前等待成本较低。`,
            `${profile.label} has low upfront ceremony and lower waiting cost before production work.`,
          )
        : score >= 70
          ? t(
              lang,
              `${profile.label} 上手成本中低，但存在基本流程约束。`,
              `${profile.label} has medium-low setup cost with basic workflow constraints.`,
            )
          : t(
              lang,
              `${profile.label} 需要角色、流程或技能库的前置投入。`,
              `${profile.label} needs upfront investment in roles, process, or skills.`,
            );
    case "multi_agent":
      return score >= 85
        ? t(
            lang,
            `${profile.label} 角色拆分、技能编排或并行推进维度较高。`,
            `${profile.label} scores high on role splitting, skill orchestration, and parallel execution.`,
          )
        : score >= 70
          ? t(
              lang,
              `${profile.label} 支持轻中度分工，多 Agent 不是唯一主轴。`,
              `${profile.label} supports light-to-medium role splitting, while multi-agent work is not the only axis.`,
            )
          : t(
              lang,
              `${profile.label} 单线推进适配更高，重型多 Agent 编排适配较低。`,
              `${profile.label} has higher single-thread fit and lower heavy multi-agent fit.`,
            );
    case "context_control":
      return score >= 85
        ? t(
            lang,
            `${profile.label} 的上下文边界和加载方式较清晰，长会话保持度较高。`,
            `${profile.label} keeps clearer context boundaries and higher long-session retention.`,
          )
        : score >= 70
          ? t(
              lang,
              `${profile.label} 有一定上下文组织能力，但仍依赖使用习惯。`,
              `${profile.label} offers some context discipline, but still depends on operator habits.`,
            )
          : t(
              lang,
              `${profile.label} 更依赖手动上下文维护。`,
              `${profile.label} depends more on manual context hygiene.`,
            );
    case "claude_fit":
      return score >= 85
        ? t(
            lang,
            `${profile.label} 在 Claude Code 里的适配分较高，组合特征更鲜明。`,
            `${profile.label} has high Claude Code fit and a clearer pairing profile there.`,
          )
        : score >= 70
          ? t(
              lang,
              `${profile.label} 在 Claude Code 里可用，但组合特征不算鲜明。`,
              `${profile.label} works in Claude Code, though the pairing profile is less distinct.`,
            )
          : t(
              lang,
              `${profile.label} 在 Claude Code 里的适配分较低。`,
              `${profile.label} has lower fit inside Claude Code.`,
            );
    case "codex_fit":
      return score >= 85
        ? t(
            lang,
            `${profile.label} 在 Codex 里的结构化执行适配分较高。`,
            `${profile.label} has high structured-execution fit inside Codex.`,
          )
        : score >= 70
          ? t(
              lang,
              `${profile.label} 在 Codex 里可用，但更看具体任务类型。`,
              `${profile.label} is usable in Codex, but fit depends more on the task shape.`,
            )
          : t(
              lang,
              `${profile.label} 在 Codex 里的适配分较低。`,
              `${profile.label} has lower fit inside Codex.`,
            );
    case "opencode_fit":
      return score >= 85
        ? t(
            lang,
            `${profile.label} 在 OpenCode 这类开放宿主里适配分较高。`,
            `${profile.label} has higher fit in open hosts like OpenCode.`,
          )
        : score >= 70
          ? t(
              lang,
              `${profile.label} 在 OpenCode 里可用，但开放宿主适配分不是最高档。`,
              `${profile.label} works in OpenCode, though open-host fit is not in the top band.`,
            )
          : t(
              lang,
              `${profile.label} 在 OpenCode 里的适配分较低。`,
              `${profile.label} has lower fit inside OpenCode.`,
            );
  }
}

function findHarness(harnessId: HarnessId): HarnessProfile {
...
function rankConfidence(hostFit: number, lang: UiLanguage): string {
  if (hostFit >= 90) {
...
function rankWhy(profile: HarnessProfile, hostId: HostId, lang: UiLanguage): string {
  ...
  return t(
    lang,
    `${profile.label} 在这个宿主里的主要优势是 ${profile.bestForZh}。`,
    `${profile.label} stands out here for ${profile.bestForEn.toLowerCase()}.`,
  );
}

function basisMetrics(profile: HarnessProfile, hostId: HostId): LeaderboardMetricId[] {
  const metrics = [
    { id: "host_fit" as const, value: profile.fitByHost[hostId] },
    { id: "specification" as const, value: profile.specification },
    { id: "planning" as const, value: profile.planning },
    { id: "execution" as const, value: profile.execution },
    { id: "context" as const, value: profile.context },
    { id: "setup" as const, value: profile.setup },
  ];
...
function buildCompareFrameworks(lang: UiLanguage): CompareFrameworkView[] {
...
function buildCompareDimensions(
  lang: UiLanguage,
  ids: CompareDimensionId[],
): CompareDimensionView[] {
  const builders: Record<CompareDimensionId, Omit<CompareDimensionView, "values">> = {
    new_project: {
      id: "new_project",
...
function hostOptions(lang: UiLanguage): HostOptionView[] {
...
function leaderboardRows(hostId: HostId, lang: UiLanguage): HostLeaderboardRowView[] {
  return HARNESSES
    .map((profile) => ({
      profile,
...
function topCards(hostId: HostId, lang: UiLanguage): HostLeaderboardTopCardView[] {
  ...
  return TOP_CARD_CONFIGS[hostId]
    ...
    .filter((value): value is HostLeaderboardTopCardView => Boolean(value));
}
function quickPicks(lang: UiLanguage): QuickPickView[] {
  return [
    {
...
export function buildConsumerHomePageView(
  ...
  return {
    ...
    hero_body: t(
      lang,
      "宿主适配、替代项、证据边界。",
      "Host fit, alternatives, evidence boundary.",
    ),
    ...
    compare_preview: {
      ...
      summary: t(
        lang,
        "新项目、现有仓库、长任务和上手速度四个切口。",
        "Four cuts: greenfield, existing repo, long task, and setup speed.",
      ),
...
export function buildHostLeaderboardPageView(
  hostId: HostId,
  lang: UiLanguage = DEFAULT_UI_LANGUAGE,
): HostLeaderboardPageView {
  const meta = HOST_META[hostId];
  ...
  return {
    ...
    hero_title: t(
      lang,
      `${t(lang, meta.titleZh, meta.titleEn)} · 首位。`,
      `${t(lang, meta.titleZh, meta.titleEn)} · top pick.`,
    ),
    hero_body: t(
      lang,
      "策展排序；验证状态见 Evidence Boards。",
      "Curated ranking; verification status in Evidence Boards.",
    ),
    host_options: hostOptions(lang),
    top_cards: topCards(hostId, lang),
    table_intro: t(
      lang,
      "策展视图：宿主适配、关键能力、风险提示、证据入口。",
      "Curated view: host fit, key capabilities, trade-offs, and evidence path.",
    ),
    rows,
    scenario_pills: t(
      lang,
      "新项目 • 现有仓库 • 长任务 • 多 Agent",
      "New project • Existing repo • Long task • Multi-agent",
    ).split(" • "),
    explanation_blocks: [
      {
        ...
        body: t(
          lang,
          "宿主差异影响适配分；按工具分榜。",
          "Host differences affect fit scores; boards remain tool-specific.",
        ),
      },
      {
        ...
        body: t(
          lang,
          "当前层是选型视图；verifier-backed 证据仍在 Evidence Boards。",
          "This layer is a selection view; verifier-backed evidence still lives in Evidence Boards.",
        ),
      },
    ],
    methodology_note: consumerProvenance(lang),
...
export function buildHarnessComparePageView(
  lang: UiLanguage = DEFAULT_UI_LANGUAGE,
): HarnessComparePageView {
  return {
    lang,
    ...
    intro: t(
      lang,
      "维度：新项目、现有仓库、长任务、上手速度、多 Agent、上下文控制，以及 Claude Code / Codex / OpenCode 适配。",
      "Dimensions: greenfield work, existing repos, long tasks, setup speed, multi-agent support, context control, and fit for Claude Code / Codex / OpenCode.",
    ),
    host_options: hostOptions(lang),
    frameworks: buildCompareFrameworks(lang),
