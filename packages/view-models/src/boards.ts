...
import type {
  ...
  BoardPageView,
  BoardRankingPolicyView,
  BoardSliceSummaryView,
  BoardStatusBreakdownView,
  HomePageView,
  RankState,
...
function boardMeta(
  ...
  title: string;
  subtitle: string;
  presentation_mode: BoardPageView["presentation_mode"];
  rules: string[];
} {
  if (boardId === "official-verified") {
    return {
      ...
      subtitle: t(
        lang,
        "默认公开结论层；仅展示 verified + published + board-eligible 结果。",
        "Default public conclusion layer; only shows verified + published + board-eligible results.",
      ),
      ...
      rules: [
        ...
        t(
          lang,
          "health / visibility / release policy 必须与 benchmark health 兼容",
          "health, visibility, and release policy must stay compatible with benchmark health",
        ),
        t(
          lang,
          "每次只显示单一 slice；不同 slice 通过切换器切换",
          "Show one slice at a time; switch slices explicitly instead of mixing them",
        ),
      ],
    };
  }
  ...
  if (boardId === "reproducibility-frontier") {
    return {
      ...
      subtitle: t(
        lang,
        "最接近官方可信结论的候选面；强调缺口与升级路径。",
        "Closest candidate surface to official conclusions; emphasizes gaps and upgrade paths.",
      ),
      presentation_mode: "queue",
      rules: [
        t(
          lang,
          "优先显示 reproduced 或 verified-pending 结果",
          "Prefer reproduced or verified-pending results",
        ),
        t(
          lang,
          "明确列出未进入 Official Board 的阻塞原因",
          "List the blockers that keep an entry out of the Official Board",
        ),
        t(
          lang,
          "每次只显示单一 slice；不把不同 comparison mode 混成一张表",
          "Show one slice at a time; do not mix different comparison modes in one table",
        ),
      ],
    };
  }
  ...
  return {
    ...
    subtitle: t(
      lang,
      "低门槛实验面；默认按最近活动展示，不伪装成官方结论。",
      "Low-friction experimental surface; shows recent activity without pretending to be official ranking truth.",
    ),
...
function observedSuccesses(record: PublicationRecord): number {
...
function rankingSortKey(record: PublicationRecord): number {
  return wilsonInterval(observedSuccesses(record), Math.max(1, record.submission.n_tasks)).low;
}

function sortRecords(records: PublicationRecord[]): PublicationRecord[] {
...
function ordinalRankAllowed(records: PublicationRecord[]): boolean {
  if (records.length < 3) {
    return false;
  }
  ...
  return records.every((record, index) => {
    ...
    const current = wilsonInterval(
      observedSuccesses(record),
      Math.max(1, record.submission.n_tasks),
    );
    ...
    const next = wilsonInterval(
      observedSuccesses(nextRecord),
      Math.max(1, nextRecord.submission.n_tasks),
    );
...
function rankingPolicy(
  ...
  return {
    ...
    separation_rule: t(
      lang,
      "只有当相邻条目的 95% Wilson 区间完全分离，且每条有效样本数达到门槛时，才允许硬名次。",
      "Ordinal ranks are allowed only when adjacent 95% Wilson intervals are fully separated and every entry meets the minimum effective sample threshold.",
    ),
    note: t(
      lang,
      "排序显示先按 Wilson 下界保守排序；区间重叠时公开成同一置信档，不强行宣称谁更强。",
      "Display order uses the Wilson lower bound conservatively; overlapping intervals are published as the same confidence band instead of forcing a stronger claim.",
    ),
  };
}

function boardEligible(boardId: BoardId, record: PublicationRecord): boolean {
  if (boardId === "official-verified") {
...
function boardAffinity(boardId: BoardId, record: PublicationRecord): boolean {
  if (boardId === "official-verified") {
  ...
  if (boardId === "reproducibility-frontier") {
    return (
      record.verification_record.trust_tier === "verified" ||
      record.verification_record.trust_tier === "reproduced"
    );
  }

  return (
    record.verification_record.trust_tier === "community" ||
    record.submission.requested_trust_tier === "community" ||
    record.verification_record.publication_state === "submitted"
  );
}

function pickRecords(boardId: BoardId, records: PublicationRecord[]): PublicationRecord[] {
  return records.filter((record) => boardEligible(boardId, record));
}

function zeroBreakdown(): BoardStatusBreakdownView {
  return {
...
function buildStatusBreakdown(
  ...
  return records.reduce<BoardStatusBreakdownView>((summary, record) => {
...
function breakdownNote(
  breakdown: BoardStatusBreakdownView,
  lang: UiLanguage,
): string {
  const notes: string[] = [];
  ...
  if (breakdown.active_blocked_entries > 0) {
    notes.push(
      t(
        lang,
        `${breakdown.active_blocked_entries} 条记录仍在 admission gate 外，说明样本存在但还没达到本板发布门槛。`,
        `${breakdown.active_blocked_entries} records still sit behind admission gates, so evidence exists but has not yet met this board's publication bar.`,
      ),
    );
  }
  ...
  if (breakdown.suspended_entries > 0) {
    notes.push(
      t(
        lang,
        `${breakdown.suspended_entries} 条记录因 dispute/correction 被暂停，不会出现在活跃榜单。`,
        `${breakdown.suspended_entries} records are suspended by dispute/correction governance and stay off active boards.`,
      ),
    );
  }
  ...
  if (breakdown.historical_entries > 0) {
    notes.push(
      t(
        lang,
        `${breakdown.historical_entries} 条记录只保留历史说明，不再参与当前比较。`,
        `${breakdown.historical_entries} records remain as historical-only evidence and no longer participate in current comparison.`,
      ),
    );
  }
  ...
  if (breakdown.hidden_entries > 0) {
    notes.push(
      t(
        lang,
        `${breakdown.hidden_entries} 条记录尚未进入公开发布面。`,
        `${breakdown.hidden_entries} records have not entered the public release surface yet.`,
      ),
    );
  }
...
function boardStateFor(
  boardId: BoardId,
  breakdown: BoardStatusBreakdownView,
  lang: UiLanguage,
  activeRecords: PublicationRecord[] = [],
): { state: RankState; reason: string } {
  const activeEligibleCount = breakdown.active_eligible_entries;
  const trackedCount =
    breakdown.active_eligible_entries +
  ...
  if (trackedCount === 0) {
    return {
      ...
      reason: t(
        lang,
        "当前 slice 仍在冷启动，还没有形成任何候选记录。",
        "This slice is still warming up and has not formed any candidate records yet.",
      ),
    };
  }
  ...
  if (activeEligibleCount === 0) {
    return {
      ...
      reason: t(
        lang,
        `当前 slice 有候选记录，但还没有活跃可展示条目。${notes}`,
        `This slice has candidate records, but no active public entries yet. ${notes}`,
      ).trim(),
    };
  }
  ...
  if (activeEligibleCount < 2) {
    return {
      ...
      reason: t(
        lang,
        `公开比较至少需要 2 个 eligible entries；当前只展示条目卡与证据摘要。${notes}`,
        `Public comparison requires at least two eligible entries, so the page currently shows cards and evidence summaries only. ${notes}`,
      ),
    };
  }
  ...
  if (activeEligibleCount === 2) {
    return {
      ...
      reason: t(
        lang,
        `当前仅支持 head-to-head 比较，不给出虚假的 ordinal rank。${notes}`,
        `Only head-to-head comparison is supported right now, so no fake ordinal ranking is shown. ${notes}`,
      ),
    };
  }
  ...
  if (boardId === "official-verified") {
    if (ordinalRankAllowed(activeRecords)) {
      return {
        ...
        reason: t(
          lang,
          `当前 slice 的 active entries 已达到样本门槛，且相邻 95% Wilson 区间完全分离，因此允许展示硬名次。${notes}`,
          `This slice meets the effective sample threshold and adjacent 95% Wilson intervals are fully separated, so ordinal ranks are allowed. ${notes}`,
        ),
      };
    }
    ...
    return {
      ...
      reason: t(
        lang,
        `在显式 separation evidence 落地前，Official Board 先 fail-closed 到 ranked_tiered。${notes}`,
        `Until explicit separation evidence exists, the Official Board fail-closes to ranked_tiered. ${notes}`,
      ),
    };
  }
  ...
  if (boardId === "reproducibility-frontier") {
    return {
      ...
      reason: t(
        lang,
        `Frontier 面用于看升级梯度与证据缺口，默认只做 tier/cluster 呈现。${notes}`,
        `The Frontier surface is for upgrade gradients and evidence gaps, so it defaults to tier/cluster presentation. ${notes}`,
      ),
    };
  }
  ...
  return {
    ...
    reason: t(
      lang,
      `Community 面默认强调趋势与分层，不作为官方权威排名。${notes}`,
      `The Community surface emphasizes trend and tiering rather than authoritative ranking. ${notes}`,
    ),
  };
}

function healthWarning(record: PublicationRecord, lang: UiLanguage): string {
  const health = record.submission.benchmark.health;

  if (health.freshness_tier === "fresh" || health.freshness_tier === "active") {
    return t(
      lang,
      `${health.freshness_tier} benchmark；公开层需遵守 release/visibility gate。`,
      `${health.freshness_tier} benchmark; the public surface must obey release and visibility gates.`,
    );
  }
  ...
  if (health.contamination_tier === "high") {
    return t(
      lang,
      "contamination 风险高，建议优先查看 research view。",
      "High contamination risk; inspect the research view first.",
    );
  }

  return t(
    lang,
    `health snapshot ${health.health_snapshot_version}`,
    `health snapshot ${health.health_snapshot_version}`,
  );
}

function chooseBaseline(records: PublicationRecord[]): PublicationRecord | undefined {
  return (
    records.find((record) => record.submission.harness.id.includes("baseline")) ??
    sortRecords(records).at(-1)
  );
}

function clusterLabel(record: PublicationRecord, lang: UiLanguage): string {
...
function sliceAnchor(record: PublicationRecord): string {
...
function comparisonModeLabel(mode: ComparisonMode, lang: UiLanguage): string {
  if (mode === "fixed_model_compare_harness") {
...
function sliceLabel(record: PublicationRecord, lang: UiLanguage): string {
  const mode = record.submission.comparison_mode;
...
function sliceContextKey(record: PublicationRecord): string {
...
type SliceGroup = {
  summary: BoardSliceSummaryView;
  activeRecords: PublicationRecord[];
  records: PublicationRecord[];
};

function singleValueOrUndefined<T>(values: T[]): T | undefined {
  if (values.length === 0) {
    return undefined;
  }
...
function buildSliceGroups(
  boardId: BoardId,
  publications: PublicationRecord[],
  lang: UiLanguage,
): SliceGroup[] {
  const grouped = new Map<string, PublicationRecord[]>();
  ...
  return [...grouped.entries()]
    .map(([sliceId, records]) => {
      ...
      const summary: BoardSliceSummaryView = {
        slice_id: sliceId,
        ...
        filters: first
          ? {
              ...
              trust_tier: singleValueOrUndefined(
                records.map((record) => record.verification_record.trust_tier),
              ),
    ...
    .sort((left, right) => {
      if (
        right.summary.status_breakdown.active_eligible_entries !==
      ...
      const leftTop =
        left.activeRecords[0]?.submission.metrics.success_rate ??
        left.records[0]?.submission.metrics.success_rate ??
        0;
      const rightTop =
        right.activeRecords[0]?.submission.metrics.success_rate ??
...
function toEntrySummary(
  record: PublicationRecord,
  baseline: PublicationRecord | undefined,
  rankState: RankState,
  rank: number,
  rankBand: string,
  boardId: BoardId,
...
export function listBoardSlices(
  boardId: BoardId,
  publications: PublicationRecord[],
...
export function buildBoardPageView(
  boardId: BoardId,
  publications: PublicationRecord[],
  requestedSliceId?: string,
  lang: UiLanguage = DEFAULT_UI_LANGUAGE,
  ...
  return {
          ...
          : Number(
              (
                (metricRecords.reduce(
                  (sum, record) => sum + record.submission.metrics.success_rate,
                  0,
                ) /
    ...
    rules: meta.rules,
    entries: picked.map((record, index) =>
      toEntrySummary(
        record,
        baseline,
        boardState.state,
        index + 1,
        rankBands.get(record.entry_id) ?? rankBandLabel(0, lang),
        boardId,
        lang,
      ),
    ),
...
export function buildAllBoardViews(
  publications: PublicationRecord[],
  lang: UiLanguage = DEFAULT_UI_LANGUAGE,
): BoardPageView[] {
  return (["official-verified", "reproducibility-frontier", "community-lab"] as BoardId[]).map((boardId) =>
    buildBoardPageView(boardId, publications, undefined, lang),
  );
}
...
export function buildHomePageView(
  ...
  return {
    ...
    hero_body: t(
      lang,
      "OHBP 网站 MVP 只显示 public evidence surface：榜单先给结论，但所有结论都必须一跳追到 verification_record、completeness-proof 与 digest metadata。",
      "The OHBP web MVP only shows the public evidence surface: boards present conclusions first, but every conclusion must trace back to verification_record, completeness-proof, and digest metadata in one hop.",
    ),
    ...
    lanes: [
      {
        ...
        why_it_exists: t(
          lang,
          "低门槛跑通协议、upload receipt 与 public evidence surface。",
          "Low-friction path for exercising the protocol, upload receipt, and public evidence surface end to end.",
        ),
      },
      {
        ...
        why_it_exists: t(
          lang,
          "默认承载 Official Verified Board 的高信任比较切片。",
          "Default home for high-trust comparison slices that can graduate to the Official Verified Board.",
        ),
      },
      {
        ...
        why_it_exists: t(
          lang,
          "保留给更复杂的 workflow / policy 研究，不打散首页叙事。",
          "Reserved for more complex workflow and policy studies without breaking the homepage story.",
        ),
      },
    ],
    protocol_objects: [
