...
function protocolObjects(lang: UiLanguage): ProtocolObjectEntry[] {
  return [
    {
      id: "manifest",
      title: "manifest.json",
      summary: t(
        lang,
        "bundle 侧唯一 join surface；v0.2 的“文件真”先从这里核对 manifest、digest 与公开投影是否一致。",
        "The only bundle-side join surface; in v0.2, the first \"file truth\" check starts here by matching the manifest, digest, and public projection.",
      ),
    ...
    },
    {
      ...
      summary: t(
        lang,
        "冻结 run-group 比较边界、attempt 计划与 requested_trust_tier；也是低成本 solo-operator flow 的上传前起点。",
        "Freezes the run-group comparison boundary, attempt plan, and requested_trust_tier; it is also the preregistration starting point for the low-cost solo-operator flow.",
      ),
    ...
    },
    {
      ...
      summary: t(
        lang,
        "平台生成；证明声明过的 run-group 是否完整披露，主要对应六道闸门里的“次数真”。",
        "Platform-generated proof showing whether the declared run-group is fully disclosed, primarily covering the \"run-count truth\" gate in the six-gate model.",
      ),
    ...
    },
    {
      ...
      summary: t(
        lang,
        "带 actor / at / reason_code / summary 的治理动作；用于把条目从自动状态推进到 disputed、corrected、invalidated 等可追溯状态。",
        "A governance action carrying actor / at / reason_code / summary; it moves an entry from the auto-derived state into traceable states such as disputed, corrected, or invalidated.",
      ),
    ...
    },
    {
      ...
      summary: t(
        lang,
        "trust_tier / publication_state / board_disposition / autonomy_mode 的平台真源；state_history 也从这里读。",
        "The platform source of truth for trust_tier, publication_state, board_disposition, and autonomy_mode; state_history is read from here as well.",
      ),
    ...
    },
    {
      ...
      summary: t(
        lang,
        "排名成立的最小上下文；v0.2/v0.3 只允许在 fixed slice 内比较，不同任务包、不同 slice 或不同榜单处置状态不得混排。",
        "The minimum context in which ranking is meaningful; v0.2/v0.3 only compares entries inside a fixed slice, and entries from different task packages, slices, or board-disposition states must not be blended together.",
      ),
...
const OBJECT_DEPENDENCIES: Record<string, string[]> = {
  manifest: [],
...
function fieldGlossary(lang: UiLanguage): ProtocolFieldGlossaryEntry[] {
  return [
    {
      field: "trust_tier",
    ...
    },
    {
      ...
      value_domain:
        "submitted | provisional | published | disputed | corrected | invalidated | rejected | archived",
      owner: t(lang, "平台 verifier", "platform verifier"),
    ...
    },
    {
      ...
      value_domain:
        "public_full | public_redacted | public_summary | sealed_pending_publication",
    ...
    },
    {
      ...
      value_domain:
        "public_immediate | delayed_until_date | delayed_until_legacy | summary_only_permanent",
    ...
    },
    {
      ...
      value_domain:
        "fixed_model_compare_harness | fixed_harness_compare_model | system_combination",
...
export function buildProtocolIndexView(
...
export function buildProtocolObjectEntry(
  objectId: string,
  lang: UiLanguage = DEFAULT_UI_LANGUAGE,
): ProtocolObjectEntry | undefined {
  return protocolObjects(lang).find((entry) => entry.id === objectId);
}

export function buildProtocolFieldEntry(
  fieldId: string,
  lang: UiLanguage = DEFAULT_UI_LANGUAGE,
): ProtocolFieldGlossaryEntry | undefined {
  return fieldGlossary(lang).find((entry) => entry.field === fieldId);
}

function searchMatches(query: string | undefined, parts: Array<string | string[] | undefined>): boolean {
  if (!query) {
    return true;
  }
...
export function buildProtocolPageView(
  rawQuery?: string,
  lang: UiLanguage = DEFAULT_UI_LANGUAGE,
): ProtocolPageView {
  ...
  const sections = [
    ...
    searchMatches(query, [section.id, section.title, section.summary, section.bullets]),
  );
  ...
  const glossary = glossaryEntries.filter((entry) =>
    searchMatches(query, [entry.field, entry.value_domain, entry.owner, entry.used_by, entry.common_misuse]),
  );
  ...
  const search_summary = query
    ? t(
        lang,
        `搜索 "${query}"：匹配 ${sections.length} 个规则分区、${object_map.length} 个对象、${glossary.length} 个字段。`,
        `Search "${query}": matched ${sections.length} rule sections, ${object_map.length} objects, and ${glossary.length} fields.`,
      )
    : undefined;
