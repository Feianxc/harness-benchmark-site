...
export function localizeAdmissionReason(reason, lang) {
    const exact = EXACT_REASON_TRANSLATIONS[reason];
    if (exact) {
        return t(lang, exact[0], exact[1]);
    }
    if (reason.startsWith("当前 trust_tier = ")) {
        return t(lang, reason, reason.replace("当前 trust_tier = ", "current trust_tier = "));
    }
    if (reason.startsWith("autonomy_mode = ")) {
        return t(lang, reason, reason);
    }
    const freshnessGateMatch = reason.match(/^benchmark freshness = ([^,，]+)[,，]\s*不触发最严格的 public surface gate$/);
    if (freshnessGateMatch) {
        const freshnessTier = freshnessGateMatch[1];
        return t(lang, reason, `benchmark freshness = ${freshnessTier}; the strictest public-surface gate is not triggered`);
    }
    return reason;
}
export function localizeAdmissionReasons(reasons, lang) {
    return reasons.map((reason) => localizeAdmissionReason(reason, lang));
}
//# sourceMappingURL=admission-i18n.js.map