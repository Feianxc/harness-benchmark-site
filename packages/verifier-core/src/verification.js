...
const FRESHNESS_RESTRICTED_TIERS = new Set([
    "fresh",
    "active",
]);
const HIDDEN_ALLOWED_VISIBILITY = new Set([
    "public_redacted",
    "public_summary",
]);
const PUBLIC_ALLOWED_VISIBILITY = new Set([
    "public_full",
    "public_redacted",
]);
const HIDDEN_ALLOWED_RELEASE_POLICIES = new Set([
    "delayed_until_date",
    "delayed_until_legacy",
    "summary_only_permanent",
]);
const ACTIVE_BOARD_PUBLICATION_STATES = new Set([
    "submitted",
    "provisional",
    "published",
]);
const PUBLICATION_STATE_TRANSITIONS = {
...
export function deriveAutonomyMode(submission) {
    const telemetry = submission.telemetry;
    if (!telemetry || !telemetry.interaction_log_complete) {
        return "interactive";
    }
    if (telemetry.tty_freeform_input_detected ||
    ...
    if (telemetry.approval_event_count > 0 &&
        telemetry.interactive_event_count === 0 &&
    ...
    if (telemetry.human_event_count === 0 &&
...
function derivePublicationState(submission, trustTier, proof) {
...
function boardDispositionFor(publicationState) {
...
function systemStateSummary(publicationState) {
    if (publicationState === "published") {
...
function validateGovernanceTransition(fromState, toState) {
...
function selectRelevantGovernanceDirectives(submission, directives) {
    return (directives ?? []).filter((directive) => {
        ...
        if (directive.public_bundle_digest &&
...
function applyGovernanceDirectives(submission, baseState, directives) {
    const stateHistory = [
        {
...
function governancePublicationBlocks(publicationState) {
...
function evaluateOfficialPolicyCompatibility(submission) {
    const satisfied = [];
    const blocked = [];
...
function boardDecision(eligible, satisfied, blocked, nextActions) {
    return {
        eligible,
...
function buildBoardAdmission(submission, proof, trustTier, publicationState, autonomyMode) {
    ...
    const frontierSatisfied = [];
    const frontierBlocked = [];
    if (proof.completeness_verdict === "complete") {
    ...
    const communityEligible = !officialEligible &&
        !frontierEligible &&
    ...
    return {
        ...
        reproducibility_frontier: boardDecision(frontierEligible, frontierSatisfied, frontierBlocked, frontierBlocked.length > 0 ? frontierBlocked : ["等待平台队列推进"]),
        community_lab: boardDecision(communityEligible && !officialEligible, communitySatisfied, communityBlocked, ["补 registration/completeness 后可升级到 reproducible_standard"]),
    };
}
function buildInteractionSummaryDigest(telemetry, autonomyMode) {
    if (!telemetry) {
        return undefined;
    }
    return sha256Digest({
...
export function createVerificationRecord(submission, proof, options) {
    ...
    const governance = applyGovernanceDirectives(submission, basePublicationState, options?.governanceDirectives);
    const boardAdmission = buildBoardAdmission(submission, proof, trustTier, governance.publicationState, autonomyMode);
    ...
    return {
        ...
        interaction_summary_digest: buildInteractionSummaryDigest(submission.telemetry, autonomyMode),
