...
const DEFAULT_HEALTH: BenchmarkHealthSnapshot = {
...
function inferProfile(requestedTrustTier: TrustTier): SubmissionProfile {
...
const CONSERVATIVE_INTERACTIVE_TELEMETRY: InteractionTelemetry = {
  human_event_count: 1,
...
function normalizeTelemetry(
  telemetry: Partial<InteractionTelemetry> | undefined,
): InteractionTelemetry | undefined {
  if (!telemetry) {
    return undefined;
  }
...
function normalizeBenchmark(
  benchmark: Partial<BenchmarkDescriptor> | undefined,
...
function normalizeParticipant(
  value: Partial<ParticipantDescriptor> | undefined,
  fallback: ParticipantDescriptor,
): ParticipantDescriptor {
  return {
...
function resolveVisibilityDefaults(
  split: BenchmarkDescriptor["split"],
): {
  evidence_channel_mode: NormalizedSubmission["evidence_channel_mode"];
  visibility_class: VisibilityClass;
  release_policy: NormalizedSubmission["release_policy"];
} {
...
async function payloadFromBundlePath(bundlePath: string): Promise<MockSubmissionPayload> {
  const content = await readFile(bundlePath, "utf8");
  const parsed = JSON.parse(content) as MockSubmissionPayload | { normalized_payload?: MockSubmissionPayload };
  return "normalized_payload" in parsed && parsed.normalized_payload
    ? parsed.normalized_payload
    : (parsed as MockSubmissionPayload);
}

export async function loadUploadPayload(
  input: MockUploadInput,
): Promise<{ payload: MockSubmissionPayload; bundlePath?: string }> {
...
export async function normalizeIncomingSubmission(
  input: MockUploadInput,
): Promise<{ normalized: NormalizedSubmission; bundlePath?: string }> {
  ...
  const submissionProfile =
    payload.submission_profile ?? inferProfile(requestedTrustTier);
  ...
  const submissionId =
    payload.submission_id ?? createDeterministicId("submission", identitySeed);
  const studyId =
    payload.study_id ??
  ...
  const taskPackageDigest =
    payload.task_package_digest ??
    sha256Digest({
      benchmark: benchmark.id,
      version: benchmark.version,
      lane: benchmark.lane_id,
  ...
  const executionContractDigest =
    payload.execution_contract_digest ??
    sha256Digest({
      benchmark: benchmark.id,
      model: model.id,
      harness: harness.id,
      comparison_mode:
        payload.comparison_mode ?? "fixed_model_compare_harness",
      budget_class: payload.budget_class ?? "standard",
    });
  const tolerancePolicyDigest =
    payload.tolerance_policy_digest ??
    sha256Digest({
      benchmark: benchmark.id,
      lane: benchmark.lane_id,
      requestedTrustTier,
  ...
  const publicBundleDigest =
    payload.public_bundle_digest ??
    sha256Digest({
      ...digestSeed,
      public_surface: {
        benchmark: benchmark.id,
  ...
  return {
    ...
    normalized: {
      ...
      evidence_channel_mode:
        payload.evidence_channel_mode ??
        visibilityDefaults.evidence_channel_mode,
      visibility_class:
        payload.visibility_class ?? visibilityDefaults.visibility_class,
      ...
      tags: payload.tags ?? [],
      submitted_at:
        payload.submitted_at ?? input.received_at ?? "2026-04-21T00:00:00.000Z",
    },
  };
}

export function previewNormalizedPayload(
  payload: MockSubmissionPayload,
): Omit<NormalizedSubmission, "submission_id" | "study_id" | "run_group_id" | "attempt_id" | "bundle_id" | "entry_id"> & {
  ...
  return {
    ...
    budget_class: payload.budget_class ?? "standard",
    evidence_channel_mode:
      payload.evidence_channel_mode ?? defaults.evidence_channel_mode,
    ...
    tolerance_policy_digest:
      payload.tolerance_policy_digest ?? "sha256:<to-be-computed>",
    execution_contract_digest:
      payload.execution_contract_digest ?? "sha256:<to-be-computed>",
    task_package_digest:
      payload.task_package_digest ?? "sha256:<to-be-computed>",
    registration_digest: payload.registration_digest ?? "sha256:<to-be-computed>",
    public_bundle_digest:
      payload.public_bundle_digest ?? "sha256:<to-be-computed>",
    sealed_audit_bundle_digest: payload.sealed_audit_bundle_digest,
    provider_release_window: payload.provider_release_window,
