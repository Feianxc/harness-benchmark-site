...
import type { CompletenessProof, NormalizedSubmission } from "./types.js";

function slotIds(prefix: string, count: number): string[] {
  return Array.from({ length: count }, (_, index) => `${prefix}-${index + 1}`);
}

function resolveTierEligibilityEffect(
  submission: NormalizedSubmission,
  proof: Omit<CompletenessProof, "tier_eligibility_effect">,
): CompletenessProof["tier_eligibility_effect"] {
  if (proof.completeness_verdict !== "complete") {
    return "blocked";
  }

  if (
    submission.repeatability_class === "pseudo_repeated" &&
...
export function createCompletenessProof(
  submission: NormalizedSubmission,
): CompletenessProof {
  ...
  const unexpectedAttempts =
    observed > expected ? slotIds("unexpected", observed - expected) : [];
  ...
  let completenessVerdict: CompletenessProof["completeness_verdict"] = "complete";
  ...
  } satisfies Omit<CompletenessProof, "tier_eligibility_effect">;
